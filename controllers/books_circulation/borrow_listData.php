<?php
    ...
    if($result){
        //定义返回的数据头
        $res = array('code' => 200,'msg' => "success",'count' => mysqli_num_rows($result1),'data'=> mysqli_fetch_all($result,MYSQLI_ASSOC));
        //输出结果
    ...
    foreach ($result as $item) {
        ...
        if($renew_backDate > $back_date){
            ...
            $leftDay = abs(round($diff / 86400));  //剩余天数
        }else{
            $diff = strtotime($back_date) - strtotime($today);
            $leftDay = abs(round($diff / 86400));  //剩余天数
        }

        //判断当还书日期等于0 并且 未归还 就不再 -1 了
        if($leftDay > 0 && $is_back == 0){
            if($key && $back_date > $today || $renew_backDate > $today){
                $sql = "update book_borrow set left_day='$leftDay' where card_id='$user_id' and book_id='$key'";
        ...
        $sender = '借阅到期提醒';
        $content = $user.'，您于'.$borrow_date.'借阅的《'.$book_name.'》还有3天期限，请在'.$back_date.'前归还或完成续借操作，逾期将受到惩罚！';
        //避免重复发送
        $i = mysqli_num_rows(mysqli_query($db_connect, "select * from sys_msg where content='$content'"));
        if($i == 0){  //查询返回条数，等于0就发送消息，1则不发送
            //发送即将到期提醒
            if($key && $leftDay == 3){
        ...
        $yz_sql = "select * from book_borrow where card_id='$user_id' and book_id='$key'";
        $yz_res = mysqli_query($db_connect, $yz_sql);
        $flag = mysqli_num_rows($yz_res);  //判断图书是否归还
        $sender1 = '图书逾期提醒';
        $content1 = $user.'，您于'.$borrow_date.'借阅的《'.$book_name.'》已经逾期，您的借阅卡已被暂停使用，请及时处理（您可以归还图书或联系管理员处理）！';
        //避免重复发送
        $i = mysqli_num_rows(mysqli_query($db_connect, "select * from sys_msg where content='$content1'"));
        if($i == 0){  //查询返回条数，等于0就发送消息，1则不发送
            //发送逾期信息
            if($flag == 1 && $leftDay < 0){
